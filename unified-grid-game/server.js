...
function loadMapData() {
    try {
        // Load 2P2G maps
        const maps2P2GPath = path.join(__dirname, 'public', 'designData', 'MapsFor2P2G.js');
        ...
        // Load 2P3G maps
        const maps2P3GPath = path.join(__dirname, 'public', 'designData', 'MapsFor2P3G.js');
...
io.on('connection', (socket) => {
    ...
    // Handle ready to start (alias for player_ready)
    ...
    socket.on('start_trial', (data) => {
        if (currentRoom && currentRoom.gameActive) {
            // The server should handle trial progression automatically
            // This is just a signal that the client is ready for the next trial
            console.log(`Client ${socket.id} ready for next trial`);
        }
    });
    ...
    // Handle new goal generated by player (for 2P3G synchronization) - LEGACY
    socket.on('new_goal_generated', (data) => {
        if (currentRoom && currentRoom.gameType === '2P3G') {
            console.log(`Player ${socket.id} generated new goal (LEGACY):`, data.newGoalPosition);

            // Check if another player is already generating a goal
...
// Add route for debug test
app.get('/debug-test', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'debug-test.html'));
});

// Add route for fix test
app.get('/test-fix', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'test-fix.html'));
});

// Add route for mode behavior test
app.get('/test-mode-behavior', (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'test-mode-behavior.html'));
});
